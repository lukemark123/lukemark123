# Hi there, I'm Luke 👋

I'm a vocational student in **Information and Communications Technology** at [Vantaa Vocational College Varia](https://varia.vantaa.fi/en) in Finland 🇫🇮.

Passionate about coding, building things from scratch, and diving into how hardware works.

## 🚀 About Me
- Studying **Tieto- ja viestintätekniikka** (ICT) — focusing on practical skills in software, networks, and emerging tech like wellbeing technology.
- Main languages: **Python** 🐍 and **C++** ⚡
- Whenever I'm bored, I fire up a new project — mixing code, creativity, and sometimes hardware tinkering.
- Currently learning more about electronics, embedded systems, and low-level stuff.

## 🛠️ Tech & Tools I Use
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?logo=c%2B%2B&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?logo=c&logoColor=black)
![Arduino](https://img.shields.io/badge/Arduino-00979D?logo=arduino&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A228B5?logo=raspberry-pi&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)

## 🔭 Current Focus
- Building unique personal projects (check my repos!)
- Exploring hardware integration with code (sensors, microcontrollers, etc.)
- Improving my skills for real-world ICT challenges

## 💻 Project
-  real-time terminal-based chat application called [Flux](https://github.com/lukemark123/Flux)
-  A secure, offline[ password manager](https://github.com/lukemark123/Offline-Password-Manager) for Windows 

## 📫 Get in Touch
- GitHub: @lukemark123 (you're already here 😄)

Thanks for stopping by! Feel free to star repos or drop suggestions.
